            ...
            try
            {
                using (var conn = new OdbcConnection("dsn=dsnAuditoria"))
                {
                    ...
                    conn.Open();
                    {
                        ...
                        {
                            cmd.CommandText = "SELECT PK_Usu_codigo FROM tbl_usuarios WHERE usu_nickname = '" + usuario + "'";
            ...
            try
            {
                using (var conn = new OdbcConnection("dsn=dsnAuditoria"))
                {
...

                    {
                        using (var cmd = conn.CreateCommand())
                        {
                            cmd.CommandText = "SELECT PK_perf_cod_encabezado as Codigo, perf_nombre as Nombre, estatus as Estado FROM tbl_perfil_encabezado";
            ...
            try
            {
                using (var conn = new OdbcConnection("dsn=dsnAuditoria"))
                {
...

                    {
                        using (var cmd = conn.CreateCommand())
                        {
                            cmd.CommandText = "SELECT PK_perf_cod_encabezado as Codigo, perf_nombre as Nombre, estatus as Estado FROM tbl_perfil_encabezado WHERE PK_Perf_cod_encabezado IN (SELECT FK_Perf_codigo from tbl_relacion_usuario_perfil WHERE FK_Usu_codigo = (SELECT PK_Usu_Codigo from tbl_usuarios where usu_nickname = '" + usuario + "'))";
                            OdbcDataAdapter m_datos = new OdbcDataAdapter(cmd);
                            ds = new DataSet();
                            m_datos.Fill(ds);
                        }
