        ...
        public Usuario obtenerObjUsuario(string usuario, string contrasena)
        {
            ...
            try
            {
                ...
                using (var conn = new OdbcConnection(/*cadenaConexion.Conexion()*/"dsn=dsnAuditoria;"))
                {
                    ...
                    using (var cmd = conn.CreateCommand())
                    {
                        cmd.CommandText = "select * from tbl_usuarios where usu_nickname='" + usuario + "' and usu_password='" + contrasena + "';";
                        cmd.ExecuteNonQuery();
        ...
        public int verUsuario(string usuario)
        {
            ...
            try
            {
                using (var conn = new OdbcConnection("dsn=dsnAuditoria;"))
                {
                    ...
                    using (var cmd = conn.CreateCommand())
                    {
                        cmd.CommandText = "select count(*) from bdauditoria.tbl_usuarios where usu_nickname='" + usuario + "';";
                        cmd.ExecuteNonQuery();
        ...
        public void nuevoUsuario(string usuario, string contrasena)
        {
            try
            {
                using (var conn = new OdbcConnection("dsn=dsnAuditoria;"))
                {
                    ...
                    using (var cmd = conn.CreateCommand())
                    {
                        cmd.CommandText = "SELECT MAX(PK_Usu_Codigo)+1 FROM bdauditoria.tbl_usuarios;";
                        cmd.ExecuteNonQuery();
                        reader = cmd.ExecuteReader();
                        if (!reader.HasRows)
                        {
                    ...
                    using (var cmd = conn.CreateCommand())
                    {
                        cmd.CommandText = "INSERT INTO `bdauditoria`.`tbl_usuarios` VALUES ("+id+",'"+usuario+"','"+contrasena+"','0');";
                        cmd.ExecuteNonQuery();
